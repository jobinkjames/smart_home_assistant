...
class FaceDetector:
    ...
    def load(self):
        ...
        print(
            f"[face_detector] {len(self._known_names)} face(s) loaded: "
    ...
    def detect(self, frame_rgb: np.ndarray) -> dict:
        ...
        small = (
            face_recognition.resize_image(
                frame_rgb, int(w * DETECTION_SCALE), int(h * DETECTION_SCALE)
            )
            if hasattr(face_recognition, "resize_image")
            else __import__("cv2").resize(
                frame_rgb, (0, 0), fx=DETECTION_SCALE, fy=DETECTION_SCALE
            )
        )
